...
class DataPreprocessingToolkit(object):
    ...
    def filter_out_low_prices(df):
        ...
        :rtype: pd.DataFrame
        """
        return df.loc[df['accommodation_price'] > 50.0]
    ...
    def add_length_of_stay(df):
        ...
        # insert length_of_stay column after date_from and date_to columns instead of just adding in order to increase readability
        insertion_index = df.columns.get_loc('date_to') + 1
        # calculate difference in days between date_from and date_to
        days_diff = (df['date_to'] - df['date_from']).dt.days
        # insert at specified index
        df.insert(insertion_index, 'length_of_stay', days_diff)
    ...
    def add_book_to_arrival(df):
        ...
        # insert book_to_arrival column after booking_time column instead of just adding in order to increase readability 
        insertion_index = df.columns.get_loc('booking_time') + 1
        # calculate difference in days between date_from and booking_date
        days_diff = (df['date_from'] - df['booking_date']).dt.days
        # insert at specified index
        df.insert(insertion_index, 'book_to_arrival', days_diff)
    ...
    def add_weekend_stay(df):
        ...
        # insert weekend_stay column after length_of_stay column instead of just adding in order to increase readability of data
        insertion_index = df.columns.get_loc('length_of_stay') + 1

        # DAYS: 0 - Monday | 1 - Tuesday | 2 - Wednesday | 3 - Thursday | 4 - Friday | 5 - Saturday | 6 - Sunday

        """
        When interval contains Friday or Saturday?
        1) Start day is Friday or Saturday (4, 5)
        2) End day is Saturday (5)
        3) Length of stay >= 6 which means that interval contains every day of week including Friday and Saturday
        4) End day is smaller than Start day which means that new week started during interval so it contains Friday or Saturday always when Start day is different than Sunday (6)
        """

        # the formula is quite complex so to increase readability I will use named variables instead of just operate on numbers
        ...
        weekend_stay = (
            ((start_day >= FRIDAY) & (start_day <= SATURDAY)) # 1)
            | (end_day >= SATURDAY) # 2)
            | (length_of_stay >= 6) # 3)
            | ((end_day < start_day) & (start_day != SUNDAY)) # 4)
        )

        df.insert(insertion_index, 'weekend_stay', weekend_stay)
    ...
    def aggregate_group_reservations(self, df):
        ...
        non_group_reservations = df.loc[df['group_id'] == "", self.sum_columns + self.mean_columns + self.mode_columns + self.first_columns]
        group_reservations = df.loc[df['group_id'] != ""]

        sums_df = group_reservations.loc[:, ["group_id"] + self.sum_columns].groupby(by = ['group_id']).sum()
        means_df = group_reservations.loc[:, ["group_id"] + self.mean_columns].groupby(by = ['group_id']).mean()
        modes_df = group_reservations.loc[:, ["group_id"] + self.mode_columns].groupby(by = ['group_id']).agg(lambda x: x.value_counts().index[0])
        first_values_df = group_reservations.loc[:, ["group_id"] + self.first_columns].groupby(by = ['group_id']).first()

        dfs_to_merge = [sums_df, means_df, modes_df, first_values_df]

        group_reservations = reduce(lambda left, right: pd.merge(left, right, on = 'group_id'), dfs_to_merge)
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        rooms_df = df.loc[:, ["room_group_id"] + ['night_price']].groupby(by = ['room_group_id']).mean()
        rooms_df['night_price'] = rooms_df['night_price'].apply(lambda x: self.map_value_to_bucket(x, self.room_segment_buckets))
        rooms_df = rooms_df.rename(columns={"night_price": "room_segment"})

        df = pd.merge(df, rooms_df, on = 'room_group_id', how = 'left')
